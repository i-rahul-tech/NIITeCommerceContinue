...
public class AdminController {
	...
	public String updateSupplierStatus(@RequestParam("uid") Long uid,@RequestParam(value = "msg", required = false) String msg1, Model map, HttpServletRequest req) {
		String referer = req.getHeader("Referer");
		Supplier supplier = supplierDao.getSupplierById(uid);
		if(msg1 != "")
		{
			map.addAttribute("msg", null);
		}
		if (supplier.getEnable().equals(true)) {
			supplier.setEnable(false);
			List<Product> product = productDao.getAllProductBySupplier(supplier);
			...
			return "redirect:" + referer;
		} else if (supplier.getEnable().equals(false)) {
	...
	public String adminAddProduct(@RequestParam("prodbrand") String brand,@RequestParam("prodname") String prodname,@RequestParam("description") String description,@RequestParam("price") long price, @RequestParam("quantity") int quantity,@RequestParam("subcategory") String cat_name, 
			Principal p, Model map, HttpServletRequest req)
	{
